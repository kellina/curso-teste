...
test('Deve listar apenas as contas do usuario "autenticado"', () => {
    return app
        .db('accounts')
        .insert([
            { name: 'Acc user #01', user_id: user.id },
            { name: 'Acc user #02', user_id: user2.id },
        ])
        .then(() => request(app)
            .get(MAIN_ROUTE)
            .set('authorization', `bearer ${user.token}`)
            .then((res) => {
                ...
                expect(res.body[0].name).toBe('Acc user #01');
            }));
});
...
test('Não deve retornar uma conta de outro usuário', () => {
    return app
        .db('accounts')
        .insert({ name: 'Acc user #02', user_id: user2.id }, ['id'])
        .then((acc) => request(app)
            .get(`${MAIN_ROUTE}/${acc[0].id}`)
            .set('authorization', `bearer ${user.token}`)
            .then((res) => {
